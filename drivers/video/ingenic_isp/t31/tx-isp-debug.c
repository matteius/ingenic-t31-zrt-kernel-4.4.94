...
int private_platform_get_irq(struct platform_device *dev, unsigned int num)
{
	...
	printk(KERN_DEBUG "private_platform_get_irq: called with num = %d for device %s\n", num, dev_name(&dev->dev));

	// Log additional device details for debugging
	if (dev->resource) {
		printk(KERN_DEBUG "Device resource: start = %pa, end = %pa\n", &dev->resource->start, &dev->resource->end);
	}
	if (dev->dev.of_node) {
		printk(KERN_DEBUG "Device has OF node: %pOF\n", dev->dev.of_node);
	}
	...
	if (irq < 0)
		printk(KERN_WARNING "private_platform_get_irq: failed to get IRQ, returned %d\n", irq);
	else
	printk(KERN_INFO "private_platform_get_irq: got IRQ %d for device %s\n", irq, dev_name(&dev->dev));
