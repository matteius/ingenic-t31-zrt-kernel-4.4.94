#include <linux/slab.h>
#include <linux/clkdev.h>
#include <linux/clk.h>
#include <linux/clk-provider.h>
#include <linux/of_address.h>
#include <linux/syscore_ops.h>
#include <dt-bindings/clock/ingenic-t31.h>
#include "clk.h"

static LIST_HEAD(clock_reg_cache_list);

void ingenic_clk_save(void __iomem *base,
struct ingenic_clk_reg_dump *rd,
unsigned int num_regs)
{
for (; num_regs > 0; --num_regs, ++rd)
rd->value = readl(base + rd->offset);
}

void ingenic_clk_restore(void __iomem *base,
const struct ingenic_clk_reg_dump *rd,
unsigned int num_regs)
{
for (; num_regs > 0; --num_regs, ++rd)
writel(rd->value, base + rd->offset);
}

struct ingenic_clk_reg_dump *ingenic_clk_alloc_reg_dump(
        const unsigned long *rdump,
        unsigned long nr_rdump)
{
    struct ingenic_clk_reg_dump *rd;
    unsigned int i;

    rd = kcalloc(nr_rdump, sizeof(*rd), GFP_KERNEL);
    if (!rd)
        return NULL;

    for (i = 0; i < nr_rdump; ++i)
        rd[i].offset = rdump[i];

    return rd;
}

struct ingenic_clk_provider *ingenic_clk_alloc_provider(struct device_node *node)
{
    struct ingenic_clk_provider *p;

    p = kzalloc(sizeof(*p), GFP_KERNEL);
    if (!p)
        return NULL;

    p->clk_data.clks = kcalloc(NR_CLKS, sizeof(struct clk *), GFP_KERNEL);
    if (!p->clk_data.clks)
        goto free_provider;

    p->clk_data.clk_num = NR_CLKS;
    p->node = node;
    p->reg_base = of_iomap(node, 0);
    if (!p->reg_base) {
        pr_err("Failed to map clock provider registers\n");
        goto free_clks;
    }

    spin_lock_init(&p->lock);

    return p;

    free_clks:
    kfree(p->clk_data.clks);
    free_provider:
    kfree(p);
    return NULL;
}

void ingenic_clk_register_provider(struct ingenic_clk_provider *p)
{
    int i;

    for (i = 0; i < p->clk_data.clk_num; i++) {
        if (IS_ERR(p->clk_data.clks[i]))
            pr_warn("Failed to register clock %d: %ld\n", i,
                    PTR_ERR(p->clk_data.clks[i]));
    }

    of_clk_add_provider(p->node, of_clk_src_onecell_get, &p->clk_data);
}

void ingenic_clk_register_fixed_factor(struct ingenic_clk_provider *p,
                                       struct ingenic_fixed_rate_clock *clks,
                                       unsigned int num)
{
    struct clk *clk;
    unsigned int i;

    for (i = 0; i < num; i++) {
        clk = clk_register_fixed_factor(NULL, clks[i].name, clks[i].parent_name,
                                        clks[i].flags, 1, clks[i].fixed_rate);
        p->clk_data.clks[clks[i].id] = clk;
    }
}


void ingenic_clk_register_mux(struct ingenic_clk_provider *p,
                              struct ingenic_mux_clock *clks,
                              unsigned int num)
{
    struct clk *clk;
    unsigned int i, ret;

    for (i = 0; i < num; i++) {
        if (clks[i].table) {
            clk = clk_register_mux_table(NULL, clks[i].name, clks[i].parent_names,
                                         clks[i].num_parents, clks[i].flags,
                                         p->reg_base + clks[i].offset,
                                         clks[i].shift, BIT(clks[i].width) - 1,
                                         clks[i].mux_flags, clks[i].table, &p->lock);
        } else {
            clk = clk_register_mux(NULL, clks[i].name, clks[i].parent_names,
                                   clks[i].num_parents, clks[i].flags,
                                   p->reg_base + clks[i].offset,
                                   clks[i].shift, clks[i].width, clks[i].mux_flags, &p->lock);
        }

        if (IS_ERR(clk)) {
            pr_err("%s: failed to register clock %s\n", __func__, clks[i].name);
            continue;
        }

        p->clk_data.clks[clks[i].id] = clk;

        if (clks[i].alias) {
            ret = clk_register_clkdev(clk, clks[i].alias, clks[i].dev_name);
            if (ret)
                pr_err("%s: failed to register lookup %s\n", __func__, clks[i].alias);
        }
    }
}

void ingenic_clk_register_bus_div(struct ingenic_clk_provider *p,
                                  struct ingenic_bus_clock *clks,
                                  unsigned int num)
{
    struct clk *clk;
    unsigned int i, ret;

    for (i = 0; i < num; i++) {
        if (clks[i].table) {
            clk = clk_register_bus_divider_table(NULL, clks[i].name,
                                                 clks[i].parent_name, clks[i].flags,
                                                 p->reg_base + clks[i].cfg_offset,
                                                 clks[i].div_shift1, clks[i].div_width1,
                                                 clks[i].div_shift2, clks[i].div_width2,
                                                 p->reg_base + clks[i].busy_offset,
                                                 clks[i].busy_shift, clks[i].ce_shift,
                                                 clks[i].div_flags, clks[i].div_flags_2,
                                                 clks[i].table, &p->lock);
        } else {
            clk = clk_register_bus_divider(NULL, clks[i].name,
                                           clks[i].parent_name, clks[i].flags,
                                           p->reg_base + clks[i].cfg_offset,
                                           clks[i].div_shift1, clks[i].div_width1,
                                           clks[i].div_shift2, clks[i].div_width2,
                                           p->reg_base + clks[i].busy_offset,
                                           clks[i].busy_shift, clks[i].ce_shift,
                                           clks[i].div_flags, clks[i].div_flags_2,
                                           &p->lock);
        }

        if (IS_ERR(clk)) {
            pr_err("%s: failed to register clock %s\n", __func__, clks[i].name);
            continue;
        }

        p->clk_data.clks[clks[i].id] = clk;

        if (clks[i].alias) {
            ret = clk_register_clkdev(clk, clks[i].alias, clks[i].dev_name);
            if (ret)
                pr_err("%s: failed to register lookup %s\n", __func__, clks[i].alias);
        }
    }
}

void ingenic_clk_register_cgu_div(struct ingenic_clk_provider *p,
                                  struct ingenic_div_clock *clks,
                                  unsigned int num)
{
    struct clk *clk;
    unsigned int i, ret;

    for (i = 0; i < num; i++) {
        if (clks[i].table) {
            clk = clk_register_cgu_divider_table(NULL, clks[i].name,
                                                 clks[i].parent_name, clks[i].flags,
                                                 p->reg_base + clks[i].offset,
                                                 clks[i].shift, clks[i].width,
                                                 clks[i].busy_shift, clks[i].en_shift, clks[i].stop_shift,
                                                 clks[i].div_flags, clks[i].table, &p->lock);
        } else {
            clk = clk_register_cgu_divider(NULL, clks[i].name,
                                           clks[i].parent_name, clks[i].flags,
                                           p->reg_base + clks[i].offset,
                                           clks[i].shift, clks[i].width,
                                           clks[i].busy_shift, clks[i].en_shift, clks[i].stop_shift,
                                           clks[i].div_flags, &p->lock);
        }

        if (IS_ERR(clk)) {
            pr_err("%s: failed to register clock %s\n", __func__, clks[i].name);
            continue;
        }

        p->clk_data.clks[clks[i].id] = clk;

        if (clks[i].alias) {
            ret = clk_register_clkdev(clk, clks[i].alias, clks[i].dev_name);
            if (ret)
                pr_err("%s: failed to register lookup %s\n", __func__, clks[i].alias);
        }
    }
}

void ingenic_clk_register_fra_div(struct ingenic_clk_provider *p,
                                  struct ingenic_fra_div_clock *clks,
                                  unsigned int num)
{
    struct clk *clk;
    unsigned int i, ret;

    for (i = 0; i < num; i++) {
        clk = clk_register_fractional_divider(NULL, clks[i].name,
                                              clks[i].parent_name, clks[i].flags,
                                              p->reg_base + clks[i].offset,
                                              clks[i].mshift, clks[i].mwidth,
                                              clks[i].nshift, clks[i].nwidth,
                                              clks[i].div_flags, &p->lock);

        if (IS_ERR(clk)) {
            pr_err("%s: failed to register clock %s\n", __func__, clks[i].name);
            continue;
        }

        p->clk_data.clks[clks[i].id] = clk;

        if (clks[i].alias) {
            ret = clk_register_clkdev(clk, clks[i].alias, clks[i].dev_name);
            if (ret)
                pr_err("%s: failed to register lookup %s\n", __func__, clks[i].alias);
        }
    }
}

void ingenic_clk_register_gate(struct ingenic_clk_provider *p,
                               struct ingenic_gate_clock *clks,
                               unsigned int num)
{
    struct clk *clk;
    unsigned int i, ret;

    for (i = 0; i < num; i++) {
        clk = clk_register_gate(NULL, clks[i].name, clks[i].parent_name,
                                clks[i].flags, p->reg_base + clks[i].offset,
                                clks[i].bit_idx, clks[i].gate_flags, &p->lock);

        if (IS_ERR(clk)) {
            pr_err("%s: failed to register clock %s\n", __func__, clks[i].name);
            continue;
        }

        p->clk_data.clks[clks[i].id] = clk;

        if (clks[i].alias) {
            ret = clk_register_clkdev(clk, clks[i].alias, clks[i].dev_name);
            if (ret)
                pr_err("%s: failed to register lookup %s\n", __func__, clks[i].alias);
        }
    }
}

#ifdef CONFIG_PM_SLEEP
static int ingenic_clk_suspend(void)
{
    struct ingenic_clock_reg_cache *reg_cache;

    list_for_each_entry(reg_cache, &clock_reg_cache_list, node)
        ingenic_clk_save(reg_cache->reg_base, reg_cache->rdump,
                         reg_cache->rd_num);
    return 0;
}

static void ingenic_clk_resume(void)
{
    struct ingenic_clock_reg_cache *reg_cache;

    list_for_each_entry(reg_cache, &clock_reg_cache_list, node)
        ingenic_clk_restore(reg_cache->reg_base, reg_cache->rdump,
                            reg_cache->rd_num);
}

static struct syscore_ops ingenic_clk_syscore_ops = {
    .suspend = ingenic_clk_suspend,
    .resume = ingenic_clk_resume,
};

static void ingenic_clk_sleep_init(void __iomem *reg_base,
                                   const unsigned long *rdump,
                                   unsigned long nr_rdump)
{
    struct ingenic_clock_reg_cache *reg_cache;

    reg_cache = kzalloc(sizeof(struct ingenic_clock_reg_cache),
                        GFP_KERNEL);
    if (!reg_cache)
        panic("could not allocate register reg_cache.\n");
    reg_cache->rdump = ingenic_clk_alloc_reg_dump(rdump, nr_rdump);

    if (!reg_cache->rdump)
        panic("could not allocate register dump storage.\n");

    if (list_empty(&clock_reg_cache_list))
        register_syscore_ops(&ingenic_clk_syscore_ops);

    reg_cache->reg_base = reg_base;
    reg_cache->rd_num = nr_rdump;
    list_add_tail(&reg_cache->node, &clock_reg_cache_list);
}

#else
static void ingenic_clk_sleep_init(void __iomem *reg_base,
const unsigned long *rdump,
unsigned long nr_rdump) {}
#endif

unsigned long _get_rate(const char *clk_name)
{
    struct clk *clk;

    clk = __clk_lookup(clk_name);
    if (!clk) {
        pr_err("%s: could not find clock %s\n", __func__, clk_name);
        return 0;
    }

    return clk_get_rate(clk);
}

void ingenic_clk_of_dump(struct ingenic_clk_provider *ctx)
{
    struct clk_onecell_data *clk_data;
    struct clk *clk;
    int i;

    clk_data = &ctx->clk_data;

    for (i = 0; i < clk_data->clk_num; i++) {
        clk = clk_data->clks[i];
        if (clk != ERR_PTR(-ENOENT)) {
            pr_info("clk->id: %d, clk->name: %s\n", i, __clk_get_name(clk));
        } else {
            pr_info("clk->id: %d, clk: %p\n", i, clk);
        }
    }
}

void ingenic_clk_add_lookup(struct ingenic_clk_provider *ctx, struct clk *clk,
                            unsigned int id)
{
    if (ctx->clk_data.clks && id)
        ctx->clk_data.clks[id] = clk;
}

void __init ingenic_clk_register_alias(struct ingenic_clk_provider *ctx,
                                       const struct ingenic_clock_alias *list,
                                       unsigned int nr_clk)
{
    struct clk *clk;
    unsigned int idx, ret;

    if (!ctx->clk_data.clks) {
        pr_err("%s: clock table missing\n", __func__);
        return;
    }

    for (idx = 0; idx < nr_clk; idx++, list++) {
        if (!list->id) {
            pr_err("%s: clock id missing for index %d\n", __func__, idx);
            continue;
        }

        clk = ctx->clk_data.clks[list->id];
        if (!clk) {
            pr_err("%s: failed to find clock %d\n", __func__, list->id);
            continue;
        }

        ret = clk_register_clkdev(clk, list->alias, list->dev_name);
        if (ret)
            pr_err("%s: failed to register lookup %s\n", __func__, list->alias);
    }
}

void __init ingenic_clk_register_fixed_rate(struct ingenic_clk_provider *ctx,
                                            const struct ingenic_fixed_rate_clock *list,
                                            unsigned int nr_clk)
{
    struct clk *clk;
    unsigned int idx, ret;

    for (idx = 0; idx < nr_clk; idx++, list++) {
        clk = clk_register_fixed_rate(NULL, list->name, list->parent_name,
                                      list->flags, list->fixed_rate);
        if (IS_ERR(clk)) {
            pr_err("%s: failed to register clock %s\n", __func__, list->name);
            continue;
        }

        ingenic_clk_add_lookup(ctx, clk, list->id);

        ret = clk_register_clkdev(clk, list->name, NULL);
        if (ret)
            pr_err("%s: failed to register clock lookup for %s", __func__, list->name);
    }
}

void __init ingenic_clk_register_fixed_factor(struct ingenic_clk_provider *ctx,
                                              const struct ingenic_fixed_factor_clock *list,
                                              unsigned int nr_clk)
{
    struct clk *clk;
    unsigned int idx;

    for (idx = 0; idx < nr_clk; idx++, list++) {
        clk = clk_register_fixed_factor(NULL, list->name, list->parent_name,
                                        list->flags, list->mult, list->div);
        if (IS_ERR(clk)) {
            pr_err("%s: failed to register clock %s\n", __func__, list->name);
            continue;
        }

        ingenic_clk_add_lookup(ctx, clk, list->id);
    }
}

void __init ingenic_clk_of_register_fixed_ext(struct ingenic_clk_provider *ctx,
                                              struct ingenic_fixed_rate_clock *fixed_rate_clk,
                                              unsigned int nr_fixed_rate_clk,
                                              const struct of_device_id *clk_matches)
{
    const struct of_device_id *match;
    struct device_node *clk_np;
    u32 freq;
    u32 index = 0;

    for_each_matching_node_and_match(clk_np, clk_matches, &match) {
        if (of_property_read_u32(clk_np, "clock-frequency", &freq))
            continue;
        fixed_rate_clk[index].fixed_rate = freq;
        index++;
    }
    ingenic_clk_register_fixed_rate(ctx, fixed_rate_clk, nr_fixed_rate_clk);
}
