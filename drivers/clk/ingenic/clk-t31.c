...
{
    struct t31_clk_provider *p;

    printk("T31 Clock Power Management Unit init!\n");

    p = ingenic_clk_alloc_provider(np);
    if (!p)
        panic("%s: unable to allocate provider.\n", __func__);

    /* Register Ext Clocks From DT */
    ingenic_clk_register_fixed_factor(p, t31_fixed_rate_ext_clks, ARRAY_SIZE(t31_fixed_rate_ext_clks));

    /* Register PLLs. */
    ingenic_clk_register_pll(p, t31_pll_clks, ARRAY_SIZE(t31_pll_clks));

    /* Register Muxs */
    ingenic_clk_register_mux(p, t31_mux_clks, ARRAY_SIZE(t31_mux_clks));

    /* Register Bus Divs */
    ingenic_clk_register_bus_div(p, t31_bus_div_clks, ARRAY_SIZE(t31_bus_div_clks));

    /* Register Divs */
    clk_div_table_generate();
    ingenic_clk_register_cgu_div(p, t31_div_clks, ARRAY_SIZE(t31_div_clks));

    /* Register Fractional Divs */
    ingenic_clk_register_fra_div(p, t31_fdiv_clks, ARRAY_SIZE(t31_fdiv_clks));

    /* Register Gates */
    ingenic_clk_register_gate(p, t31_gate_clks, ARRAY_SIZE(t31_gate_clks));

    ingenic_clk_register_provider(p);
