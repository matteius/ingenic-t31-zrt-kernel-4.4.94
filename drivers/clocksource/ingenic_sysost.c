...
static int __init ingenic_ost_probe(struct device_node *np)
{
	...
	ost->clocks = kzalloc(sizeof(*ost->clocks) + 
	                      sizeof(ost->clocks->hws[0]) * ost->soc_info->num_channels, 
	                      GFP_KERNEL);
	if (!ost->clocks) {
		ret = -ENOMEM;
		goto err_clk_disable;
...
CLOCKSOURCE_OF_DECLARE(x1000_ost_cs, "ingenic,x1000-ost", ingenic_ost_init);
CLOCKSOURCE_OF_DECLARE(x1800_ost_cs, "ingenic,x1800-ost", ingenic_ost_init);
CLOCKSOURCE_OF_DECLARE(t31_ost_cs, "ingenic,t31-ost", ingenic_ost_init);
CLOCKSOURCE_OF_DECLARE(t40_ost_cs, "ingenic,t40-ost", ingenic_ost_init);
