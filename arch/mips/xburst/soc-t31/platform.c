...
unsigned long ispmem_base = 0x13300000;
EXPORT_SYMBOL(ispmem_base);

unsigned long ispmem_size = 0x100000;
...
static inline void __iomem * private_ioremap(phys_addr_t offset, unsigned long size)
{
...
static void private_raw_mutex_init(struct mutex *lock)
{
	__mutex_init(lock, NULL, NULL);
}

static int private_request_module(bool wait, const char *fmt, char *s)
{
	return __request_module(wait, fmt, s);
}
...
// Wrapper functions to match the expected function pointer types
...
static int private_misc_deregister(struct miscdevice *misc)
{
	misc_deregister(misc);
	return 0;
}

static int private_dev_set_drvdata(struct device *dev, void *data)
{
	dev_set_drvdata(dev, data);
	return 0;
}

static int private_clk_is_enabled(struct clk *clk)
{
	return __clk_is_enabled(clk) ? 1 : 0;
}

static int private_jzgpio_set_func(enum gpio_port port, enum gpio_function func, unsigned long flags)
{
	return jzgpio_set_func((int)port, func, flags);
}


static int jzgpio_ctrl_pull(enum gpio_port port, int pull, unsigned long flags)
{
	// This is a stub function, it does nothing and returns 0.
	return 0;
}
...
static void private_kfree(void *ptr) {
	kfree(ptr);
}
...
static struct jz_driver_common_interfaces private_funcs = {
	...
	.tx_mutex_init = private_raw_mutex_init,

	/* clock interfaces */
	.priv_clk_get = clk_get,
	.priv_clk_enable = clk_enable,
	.priv_clk_is_enabled = private_clk_is_enabled,
	...
	.priv_jzgpio_set_func = private_jzgpio_set_func,
	...
	.tx_request_module = private_request_module,
	...
	.priv_wait_event_interruptible = private_wait_event_interruptible,
	.priv_wake_up_all = private_wake_up_all,
	.priv_wake_up = private_wake_up,
	.priv_init_waitqueue_head = private_init_waitqueue_head,
	.priv_wait_for_completion_timeout = wait_for_completion_timeout,

	/* misc */
	.priv_misc_register = misc_register,
	.priv_misc_deregister = private_misc_deregister,
	/* proc */
	...
	.priv_proc_remove = private_proc_remove,
	.priv_seq_printf = private_seq_printf,
	.priv_simple_strtoull = private_simple_strtoull,

	/* kthread */
	.priv_kthread_should_stop = kthread_should_stop,
	.priv_kthread_run = mythread_run,
	.priv_kthread_stop = kthread_stop,
	.priv_kmalloc = kmalloc,
	.priv_kfree = private_kfree,
