...
static int dmmu_make_present(unsigned long addr, unsigned long end)
{
    ...
    // Set GUP flags based on VMA flags
    ...
    pages = (end - addr + PAGE_SIZE - 1) / PAGE_SIZE;
    ret = get_user_pages(addr, pages, gup_flags, NULL, NULL);

    // Restore the original page protection flags
    ...
    return ret == pages ? 0 : -1;  // Check if all requested pages were processed
}
